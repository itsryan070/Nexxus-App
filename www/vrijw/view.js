        ...
        html += "<div class='visability ui-content ui-body-a' id='data' data-role='content' data-theme='a' role='main'>"
          + "<a onclick='c.closingPopup()' style='position:relative; float: right;margin:0'  data-role='button'  class='ui-btn ui-shadow ui-corner-all ui-icon-delete ui-btn-icon-notext ui-btn-right ui-rood' ></a>"
               ...
               +" <table id='info' data-role='table' class='ui-responsive table-stroke ui-table ui-table-reflow'>"
                    + "<tbody >"
                        + "<tr>"  
                            + "<td id='stad' ><b class='ui-table-cell-label' style='width: 30vw;'> Stad: </b></td><td style='min-width:50vw'>" + sup.city + "</td>"
                        + "</tr>"
                        + "<tr>" 
                            +" <td id='straat'><b class='ui-table-cell-label'  style='width: 20vw;'> Straat: </b></td><td style='min-width:50vw'>" + sup.street + "</td>"
                        +"</tr>"
                        +"<tr> "
                            +"<td id='datum'><b class='ui-table-cell-label' style='width: 30vw;'> Datum: </b></td><td style='min-width:50vw'>" + this.parseTSDate(task.order_date) + "</td>"
                        +"</tr>"
                        + "<tr>" 
                            + "<td id='wat'><b class='ui-table-cell-label'  style='width: 30vw;'> Hoeveelheid: </b> </td><td style='min-width:50vw'>"+ totalproducts + "</td>"
                        +"</tr>"
                            +"<tr><td id='tijd'><b class='ui-table-cell-label'  style='width: 30vw;'> Tijd: </b></td><td style='min-width:50vw'>"+ this.parseTSTime(task.order_date) +"</td>"
                        +"</tr>"
                        +"<tr>"
                            +"<td id='contact'><b class='ui-table-cell-label'  style='width: 30vw;'> Contact: </b></td><td style='min-width:50vw'>"+ sup.name +" </td>"
                        +"</tr>"
                        +"<tr>"
                            +"<td id='tel'><b class='ui-table-cell-label' style='width: 30vw;'> Telefoon: </b></td><td style='min-width:50vw'>"+ sup.phone +" </td>"
                        + "</tr>"
                    +"</tbody>"; 
                +"</table> "
        +"</div>";
    ...
    showPopupRe(){
        ...
        popup += '<div class="ui-popup-screen ui-overlay-inherit in" id="reden-screen"></div> '
                + '<div class="ui-popup-container pop in ui-popup-active" id="reden-popup" style="max-width: 330px; top: 171px; left: 29px;">'
                    ...
                    + '<form>'
                        +'<div class="ui-radio">'
                            +'<label class="ui-radio-pop ui-btn ui-corner-all ui-btn-inherit ui-btn-icon-left ">Ziek'
                            +'</label>'
